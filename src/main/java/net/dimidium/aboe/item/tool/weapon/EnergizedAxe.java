...
public class EnergizedAxe extends AxeItem implements IItemFEStorage, IItemTab
{
    ...
    public boolean isCorrectToolForDrops(ItemStack stack, BlockState state)
    {
        ...
        {
            return this.getCurrentFE(stack) >= ConfigurationHandler.BEGINNER_ENERGIZED_AXE_DRAIN_SPEED.get();
        }

        else if(stack.is(ItemRegistry.INTERMEDIATE_ENERGIZED_AXE.get()))
        {
            return this.getCurrentFE(stack) >= ConfigurationHandler.INTERMEDIATE_ENERGIZED_AXE_DRAIN_SPEED.get();
        }

        else if(stack.is(ItemRegistry.ADVANCED_ENERGIZED_AXE.get()))
        {
            return this.getCurrentFE(stack) >= ConfigurationHandler.ADVANCED_ENERGIZED_AXE_DRAIN_SPEED.get();
        }

        else if(stack.is(ItemRegistry.EXPERT_ENERGIZED_AXE.get()))
        {
            return this.getCurrentFE(stack) >= ConfigurationHandler.EXPERT_ENERGIZED_AXE_DRAIN_SPEED.get();
        }
    ...
    {
        if(item.is(ItemRegistry.BEGINNER_ENERGIZED_AXE.get()))
        {
            this.extractFE(item, ConfigurationHandler.BEGINNER_ENERGIZED_AXE_DRAIN_SPEED.get(), EnergyAction.EXECUTE);
            return true;
        }

        else if(item.is(ItemRegistry.INTERMEDIATE_ENERGIZED_AXE.get()))
        {
            this.extractFE(item, ConfigurationHandler.INTERMEDIATE_ENERGIZED_AXE_DRAIN_SPEED.get(), EnergyAction.EXECUTE);
            return true;
        }

        else if(item.is(ItemRegistry.ADVANCED_ENERGIZED_AXE.get()))
        {
            this.extractFE(item, ConfigurationHandler.ADVANCED_ENERGIZED_AXE_DRAIN_SPEED.get(), EnergyAction.EXECUTE);
            return true;
        }

        else if(item.is(ItemRegistry.EXPERT_ENERGIZED_AXE.get()))
        {
            this.extractFE(item, ConfigurationHandler.EXPERT_ENERGIZED_AXE_DRAIN_SPEED.get(), EnergyAction.EXECUTE);
            return true;
        }
    ...
    public double getMaxInput(ItemStack item)
    {
        if(item.is(ItemRegistry.BEGINNER_ENERGIZED_AXE.get()))
        {
            return ConfigurationHandler.BEGINNER_ENERGIZED_AXE_CHARGING_SPEED.get();
        }

        else if(item.is(ItemRegistry.INTERMEDIATE_ENERGIZED_AXE.get()))
        {
            return ConfigurationHandler.INTERMEDIATE_ENERGIZED_AXE_CHARGING_SPEED.get();
        }

        else if(item.is(ItemRegistry.ADVANCED_ENERGIZED_AXE.get()))
        {
            return ConfigurationHandler.ADVANCED_ENERGIZED_AXE_CHARGING_SPEED.get();
        }

        else if(item.is(ItemRegistry.EXPERT_ENERGIZED_AXE.get()))
        {
            return ConfigurationHandler.EXPERT_ENERGIZED_AXE_CHARGING_SPEED.get();
        }
